...
class DayData:
    def __init__(self):
        self.day_number = 0
        self.produced_products = 0
        self.faulty_products = 0
        self.incomplete_products = 0
        self.planned_products_left = 0
        # [Tiempo Total de cada Work Station, Numero de veces usada Workstation]
        self.work_station_times = None
        # sacando con el atributo anterior WrkStationTimes
        self.work_stations_average_times = [0, 0, 0, 0, 0, 0]


DailyProductions = []

for i in range(30):
    DailyProductions.append(DayData())
    ...
    def status(self, value: ProductStatus) -> None:
        ...
        if (self._status == ProductStatus.PRODUCING and self._startClock == 0):
            ...
            debugLog(Debug.DEBUG, 'The product %06d started production at %.2f' % (self._id, self._startClock))
                ...
                self._status == ProductStatus.DONE or self._status == ProductStatus.FAIL or self._status == ProductStatus.ABORT):
...
class Workstation(object):
    ...
    def processProd(self) -> simpy.Process:
        try:
            ...
            self.work_time = working_time
            # print(f"WORKSTATION {self.id} SAYS: Working Time: {working_time} in product {self.product._id}")
...
class Factory(object):
    def __init__(self, env: simpy.Environment) -> None:
        ...
        self._workstations = []
        # [Tiempo Total de cada Work Station, Numero de veces usada Workstation]
        self._workstations_times = [[],[],[],[],[],[]]
        self._workstations_average_times = [0, 0, 0, 0, 0, 0]
        self._storage = []
    ...
    def getProductsForDay(self):
        done = sum(1 for i in self._storage if i._status == ProductStatus.DONE)
        fail = sum(1 for i in self._storage if i._status == ProductStatus.FAIL)
        incomplete = sum(1 for i in self._storage if i._status == ProductStatus.INCOMPLETE)
        return [done, fail, incomplete]

    def setWorkStationAverageTimes(self):
        for idx, times in enumerate(self._workstations_times):
            self._workstations_average_times[idx] = sum(times) / len(times)


    def getWorkStationAverageTimes(self):
        return self._workstations_average_times

    def getWorkStationTimesArray(self):
        return self._workstations_times
    ...
    def orderProduct(self, id: int) -> simpy.Process:
        ...
        while not prod.isDone:
            ...
            with station.unit.request() as wrkProcess:
                ...
                self._workstations_times[idx].append(station.work_time)
                # print(f"FACTORY SAYS WRKSTATION {idx + 1}: {station.work_time} on product {prod._id}")
...
def main() -> None:
    totalProducts = 5000
    # len(DailyProductions)
    for day in range(len(DailyProductions)):
        ...
        print(factory)
        DayProduction = factory.getProductsForDay()
        DailyProductions[day].day_number = day
        DailyProductions[day].produced_products = DayProduction[0]
        DailyProductions[day].faulty_products = DayProduction[1]
        DailyProductions[day].incomplete_products = DayProduction[2]
        totalProducts = totalProducts - DayProduction[0] - DayProduction[1]
        print(totalProducts)
        DailyProductions[day].planned_products_left = totalProducts
        #Get times of machines
        DailyProductions[day].work_station_times = factory.getWorkStationTimesArray()
        # Getting Average times for each machine
        factory.setWorkStationAverageTimes()
        DailyProductions[day].work_stations_average_times = factory.getWorkStationAverageTimes()
        print(f"Day number: {DailyProductions[day].day_number}, Products Left = {DailyProductions[day].planned_products_left}, Average times {DailyProductions[day].work_stations_average_times}")
